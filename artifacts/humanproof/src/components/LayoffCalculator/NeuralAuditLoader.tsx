// NeuralAuditLoader.tsx — Enterprise-grade neural intelligence engine for the
// HumanProof Layoff Audit. The center stage renders a unified, multi-system
// visualization that communicates a sophisticated AI continuously collecting,
// streaming, processing and analyzing millions of live career-risk signals:
//
//   1. SIGNAL INTAKE FIELD — particles streamed in from the edges (raw signals
//      being harvested from companies, markets, hiring, layoffs, mobility…)
//   2. DEEP NEURAL NETWORK — multi-layer net with curved weighted synapses and
//      activations propagating left→right toward a risk-prediction core
//   3. PATTERN-DETECTION SWEEP — a rotating analysis beam that lights up the
//      neurons it crosses (hidden-pattern detection)
//   4. RISK CONVERGENCE CORE — the output neuron rendered as a pulsing core with
//      concentric rings + a converging burst as predictions resolve
//   5. DATA-FLOW STREAMS — bright packets flowing between processing stages
//
// Same props + portal behaviour as before, so it remains a drop-in loader.
...
const SECTORS = [
  ...
  { id: '04', name: 'AI Adoption',         base: 0.82, warn: true  },
...
const STATUS_MESSAGES = [
  'Initializing neural prediction engine…',
  'Harvesting live signal streams · global',
  'Ingesting SEC 8-K · WARN filings',
  'Embedding company signal vectors',
  'Propagating activations through hidden layers',
  'Detecting hidden layoff patterns',
  'Correlating workforce + market movements',
  ...
  'Validating against 4,200+ historical events',
  'Calibrating prediction confidence intervals',
  'Synthesizing personalized career intelligence',
];

const PATTERN_CALLOUTS = [
  'PATTERN · hiring-freeze precursor',
  'PATTERN · budget-cut signature',
  'PATTERN · silent restructuring',
  'PATTERN · exec-exit cascade',
  'PATTERN · sector contagion',
  'PATTERN · AI displacement vector',
  'CLUSTER · peer-layoff correlation',
  'ANOMALY · headcount divergence',
];

const TICKER_SIGNALS = [
  { name: 'Financial Health',    base: 85, tone: 'alert'   },
  { name: 'Workforce Telemetry', base: 81, tone: 'alert'   },
  { name: 'Layoff Disclosures',  base: 74, tone: 'warn'    },
  { name: 'AI Adoption',         base: 86, tone: 'alert'   },
  ...
  { name: 'Internal Mobility',   base: 61, tone: 'neutral' },
];

const PHASES = ['Ingest', 'Embed', 'Reason', 'Forecast'];
...
export const NeuralAuditLoader: React.FC<Props> = ({
  ...
  const labelText = companyName ? `Analyzing ${companyName}` : 'Layoff Audit · Neural Engine';
  ...
  return createPortal(
    <div className="nal-root" role="status" aria-label="Analyzing company data with neural intelligence engine">
      ...
      <div className="nal-topbar" aria-hidden="true">
        ...
        <span className="nal-engine">Layoff Audit<span className="sub"> Neural Engine · v4.2</span></span>
      ...
      <div className="nal-ticker" aria-hidden="true">
        <div className="tlead"><span className="tldot" />Live Signals</div>
      ...
      <div className="nal-hud tr" aria-hidden="true"><span>INFERENCE · TIER-3</span><span className="tick"/></div>
      ...
      <div className="nal-body">
        ...
        <div className="nal-center-col">
          ...
          <div className="nal-card nal-panel">
            ...
            <div className="nal-meta" aria-hidden="true">
              ...
              <span><b>{companies.toLocaleString()}</b> companies</span>
        ...
        <div className="nal-right-col" aria-hidden="true">
          ...
          <div className="nal-card nal-gauges">
            {[
              { id:'nalG1', off:g1off, val:`${Math.round(g1*100)}%`, label:'Index 01', name:'Prediction Confidence', sub:'Bayesian · 12.4k features', c1:'#6fd8ff', c2:'#7af7c0' },
              { id:'nalG2', off:g2off, val:(g2*10).toFixed(1),       label:'Index 02', name:'Layoff Risk Index',      sub:'7-day rolling · normalized', c1:'#ffb547', c2:'#ff6680' },
              { id:'nalG3', off:g3off, val:`${Math.round(g3*100)}%`, label:'Index 03', name:'Inference Signal',       sub:'SNR · multi-source fusion',  c1:'#9b7bff', c2:'#6fd8ff' },
            ].map(g => (
