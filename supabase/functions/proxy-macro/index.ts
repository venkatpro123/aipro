...
// WS10 — withRun stamps pipeline_runs with the x-request-id carried by invokeEdgeFunction.
...
Deno.serve((req) =>
  // WS10 — withRun handles OPTIONS preflight, span emission, and
  // pipeline_runs writes. The handler body is the original logic;
  // outer OPTIONS short-circuit is now subsumed by withRun.
  withRun("proxy-macro", req, async (_run) => {
