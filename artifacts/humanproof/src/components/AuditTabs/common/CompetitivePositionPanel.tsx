  ...
  return (
    <div className="relative w-28 h-28">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="text-2xl font-bold text-white/90">{percentile}</div>
        <div className="text-[10px] text-white/40 -mt-0.5">percentile</div>
      </div>
    </div>
  );
}
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        <div className="flex items-center gap-1.5 w-28 flex-shrink-0">
          ...
          <span className="text-xs text-white/60 truncate">{dim.label.split(' ')[0]}</span>
        </div>
        <div className="flex-1 h-1.5 rounded-full bg-white/[0.07] relative">
          <motion.div
            initial={{ width: 0 }}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          <span className="text-[10px] text-white/35">{gap.deadline}</span>
          ...
          <span className="text-[10px] text-white/20">·</span>
          <span className="text-[10px] text-white/35">ROI: {gap.roiScore.toFixed(1)}x</span>
        </div>
        <p className="text-xs text-white/40 mt-1 leading-snug">{gap.expectedOutcome}</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="text-sm text-white/75">{edge.userStrength}</div>
        <div className="text-xs text-white/40 mt-0.5">vs. {edge.benchmarkMedian}</div>
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-sm font-semibold text-white/90 leading-snug mb-1.5">
              {position.headline}
            </h3>
            <p className="text-xs text-white/55 leading-relaxed">{position.narrative}</p>
            <div className="flex items-center gap-2 mt-2.5">
              <ArrowUpRight className="w-3.5 h-3.5 text-white/30" />
              <span className="text-xs text-white/50">
                Target: <span className="font-semibold text-white/80">{position.targetPercentile}th percentile</span>
      ...
      >
        <div className="text-xs font-medium text-white/40 uppercase tracking-wide mb-3">
          ...
          <span className="text-white/20 ml-1.5 normal-case">(vertical line = benchmark median)</span>
        </div>
              ...
              >
                ...
                {position.gaps.length > 4 && (
                  <button
                    ...
                    className="flex items-center gap-1 mt-1 mb-2 text-xs text-white/35 hover:text-white/60 transition-colors"
                  >
        ...
        >
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              <p className="text-sm text-white/75 leading-relaxed">{position.uniquePositioning}</p>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {position.dataLimitations.length > 0 && (
        <div className="text-[10px] text-white/25 space-y-0.5 pt-1">
